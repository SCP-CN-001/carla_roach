...
def get_color_validity(
    waypoint_transform, scenario_transform, scenario_type, scenario_index, debug
):
    """
    ...
    dyaw = (
        float(waypoint_transform.rotation.yaw) - scenario_transform.rotation.yaw
    ) % 360

    if dpos > DIST_THRESHOLD:
        if not debug:
            print(
                "WARNING: Found a scenario with the wrong position "
    ...
    if dyaw > ANGLE_THRESHOLD and dyaw < (360 - ANGLE_THRESHOLD):
        if not debug:
            print(
                "WARNING: Found a scenario with the wrong orientation "
                "(Type: {}, Index: {})".format(scenario_type, scenario_index)
            )
    ...
    for scenario_data in town_data:
        ...
        for i in range(len(scenario_list)):
            ...
            world.debug.draw_string(
                scenario_location + debug_str_height,
                str(i),
                False,
                carla.Color(0, 0, 0),
                1000,
            ...
            if debug:
                ...
                spectator.set_transform(
                    carla.Transform(
                        new_location + carla.Location(z=50), carla.Rotation(pitch=-90)
                    )
                )
                input(
                    " New Scenario [{}/{}] at (x={}, y={}, z={}, yaw={}). Press Enter to continue".format(
                        i,
                        len(scenario_list) - 1,
...
def validate_scenarios(world, town_data, tmap, scenarios, debug):
    ...
    for scenario_data in town_data:
        ...
        for i in range(len(scenario_list)):
            ...
            scenario_wp = tmap.get_waypoint(scenario_location)
            color = get_color_validity(
                scenario_wp.transform, scenario_transform, scenario_type, i, debug
            )
            ...
            if debug:
                ...
                spectator.set_transform(
                    carla.Transform(
                        scenario_location + carla.Location(z=50),
                        carla.Rotation(pitch=-90),
                    )
                )
                input(
                    " Scenario [{}/{}] at (x={}, y={}, z={}, yaw={}). Press Enter to continue".format(
                        i,
                        len(scenario_list) - 1,
...
def draw_scenarios(world, town_data, scenarios, debug):
    ...
    for scenario_data in town_data:
        ...
        end_color = "\x1b[0m"
        true_color = (
            "\x1b[38;2;" + str(color.r) + ";" + str(color.g) + ";" + str(color.b) + "m"
        )
        final_message += "{}{} is colored as {}{}\n".format(
            true_color, scenario_type, SCENARIO_COLOR[scenario_type][1], end_color
        )
...
def main():
    ...
    parser.add_argument("--draw-scenarios", nargs="+", help="Draw the scenarios.")
    parser.add_argument(
        "--validate-scenarios",
        nargs="+",
    ...
    parser.add_argument("--load-town", help='Loads a specific town (example "Town01")')
    parser.add_argument("--reload-town", action="store_true", help="Reloads the town")
