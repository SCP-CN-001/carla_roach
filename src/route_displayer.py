...
def draw_keypoint(world, location):
    ...
    string = (
        "("
        + str(round(location.x, 1))
        + ", "
        + str(round(location.y, 1))
        + ", "
        + str(round(location.z, 1))
        + ")"
    )
...
def main():
    ...
    argparser.add_argument(
        "-f", "--file", required=True, help="File at which to place the scenarios"
    )
    argparser.add_argument("-sr", "--show-route", help="Shows the given route")
    argparser.add_argument(
        "-sa", "--show-all", action="store_true", help="Shows all the routes"
    )
    argparser.add_argument(
        "-sk",
        "--show-keypoints",
        action="store_true",
        help="Shows the keypoints that define the route",
    )
    argparser.add_argument(
        "-ss",
        "--show-scenarios",
        action="store_true",
        help="Shows the scemarios part of the route",
    )
