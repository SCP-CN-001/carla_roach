...
def main():
    ...
    argparser.add_argument(
        "--show", action="store_true", help="Print the results on stdout"
    )
    args = argparser.parse_args()

    if not args.endpoint and not args.show:
        print(
            "No output method was selected. Use either '--endpoint', or '--show' to get the route results"
        )
        sys.exit(0)
    ...
    for route in root.iter("route"):
        ...
        if route_town not in MAPS_LOCATIONS:
            print(
                f"Ignoring route '{route_id}' as it uses an unknown map '{route_town}"
            )
            continue
