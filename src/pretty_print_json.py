...
def prettify_json(args):
    ...
    if records_table:
        ...
        for route in records_table:
            route_completed_kms = (
                0.01
                * route["scores"]["Route completion"]
                * route["meta"]["Route length"]
                / 1000.0
            )
            metrics_route = [
                [key, "{:.3f}".format(values), ""]
                for key, values in route["scores"].items()
            ]
            infractions_route = [
                [
                    key,
                    "{:.3f} ({} occurrences)".format(
                        len(values) / route_completed_kms, len(values)
                    ),
                    "\n".join(values),
                ]
            ...
            times = [
                [
                    "Game duration",
                    "{:.3f}".format(route["meta"]["Game duration"]),
                    "seconds",
                ],
                [
                    "System duration",
                    "{:.3f}".format(route["meta"]["System duration"]),
                    "seconds",
                ],
            ]

            route_completed_length = [
                ["distance driven", "{:.3f}".format(route_completed_kms), "Km"]
            ]
            ...
            list_statistics.extend(
                [*metrics_route, *infractions_route, *times, *route_completed_length]
            )
            list_statistics.extend([["", "", ""]])

        list_statistics.extend(
            [["Total game duration", "{:.3f}".format(total_duration_game), "seconds"]]
        )
        list_statistics.extend(
            [
                [
        ...
        list_statistics.extend(
            [["Total distance driven", "{:.3f}".format(total_route_length), "Km"]]
        )
