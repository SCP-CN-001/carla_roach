...
def check_duplicates(route_ids):
    ...
    for id in route_ids:
        if route_ids.count(id) > 1:
            raise ValueError(
                f"Stopping. Found that the route {id} has more than one record"
            )
...
def check_missing_data(route_ids):
    ...
    for id in route_ids:
        ...
        if total_int - prev_total_int > 1:
            raise ValueError(
                f"Stopping. Missing some data as the ids jumped from {prev_id} to {id}"
            )
...
def main():
    ...
    argparser.add_argument(
        ...
        nargs="+",
        required=True,
        help="path to all the files containing the partial results",
    )
    argparser.add_argument(
        "-e",
        "--endpoint",
        required=True,
        help="path to the endpoint containing the joined results",
    )
    ...
    for file in args.file_paths:
        ...
        if data["sensors"]:
            if not sensors:
                ...
                raise ValueError(
                    "Stopping. Found two files with different sensor configurations"
                )

    route_ids.sort(key=lambda x: (int(x.split("_")[1]), int(x.split("_rep")[-1])))
