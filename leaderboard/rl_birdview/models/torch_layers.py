...
class XtMaCNN(nn.Module):
    ...
    def __init__(self, observation_space, features_dim=256, states_neurons=[256]):
     ...
     #   print(f"/rl_birdview/models/torch_layers.py:n_input_channels:{n_input_channels}")
    ...
    def forward(self, birdview, state):
        # birdview = birdview.unsqueeze(0) #0710  old_distribution
        # state = state.unsqueeze(0)
      #  print(f"birdview--_forward：{birdview.shape}和state--_forward:{state.shape}")
      #  print(f"用的不是autoagent的")
        x = self.cnn(birdview)
...
class ImpalaCNN(nn.Module):
    ...
    def forward(self, birdview, state):
        ...
        # birdview = birdview.unsqueeze(0) #0710
        # state = state.unsqueeze(0)#0710
