...
import signal
import sys
sys.path.append("/home/vci-1/XT/leaderV2/leaderboard/scenario_runner")

from srunner.scenariomanager.carla_data_provider import *
from srunner.scenariomanager.timer import GameTime
from srunner.scenariomanager.watchdog import Watchdog
sys.path.append("/home/vci-1/XT/leaderV2/leaderboard/")
sys.path.append("/home/vci-1/XT/leaderV2/leaderboard/leaderboard")
sys.path.append("/home/vci-1/XT/leaderV2/leaderboard/leaderboard/scenarios")
from scenarios.scenario_manager import ScenarioManager
from scenarios.route_scenario import RouteScenario
from envs.sensor_interface import SensorConfigurationInvalid
from autoagents.agent_wrapper import AgentError, validate_sensor_configuration
from utils.statistics_manager import StatisticsManager, FAILURE_MESSAGES
...
class LeaderboardEvaluator(object):
    ...
    def _setup_simulation(self, args):
        ...
        settings = carla.WorldSettings(
           ...
           # spectator_as_ego = False  j居然是没有这个接口？
        )
        client.get_world().apply_settings(settings)
        print(f"args.traffic_manager_port:{args.traffic_manager_port}")
        #netstat -tulnp | grep 8000 找到谁占用端口，以及sudo kill -9干掉这个占用得程序
    ...
    def run(self, args):
        ...
        """
        #这个实例化的时候就加载了xml文件route_configurations，args.routes就是xml文件
        ...
        crashed = False
        while route_indexer.peek() and not crashed:  #只要xml文件里面还有东西或者是没撞上，那就继续。

            # Run the scenario
            #config就是从下面这个路径读取的那个xml路线场景文件拿出来的，然后用_load_and_run_scenario函数来处理这些配置
        #/home/vci-1/XT/leaderV2/leaderboard/leaderboard/utils/route_indexer.py
            config = route_indexer.get_next_config()
            crashed = self._load_and_run_scenario(args, config) #是在这里加载了xml的那些路线场景任务吗？
