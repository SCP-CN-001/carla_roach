...
def get_sampled_topology(map_topology, resolution):
    ...
    for segment in map_topology:
        ...
        if wp1.transform.location.distance(endloc) > resolution:
            ...
            print(f"wp1:{wp1}")
            print(f"wp1.next(resolution)[0]:{wp1.next(resolution)[0]}")
            print(f"seg_dict['path']:{seg_dict['path']}")
            print(f"wp1_type:{type(wp1)}")
            print(f"wp1.next(resolution)[0]:{type(wp1.next(resolution)[0])}")
            print(f"seg_dict['path']:{type(seg_dict['path'])}")
