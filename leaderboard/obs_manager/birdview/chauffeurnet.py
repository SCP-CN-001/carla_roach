...
import h5py
import sys
sys.path.append("/home/vci-1/XT/leaderV2/leaderboard/leaderboard/obs_manager/")
sys.path.append("/home/vci-1/XT/leaderV2/leaderboard/leaderboard/")
sys.path.append("/home/vci-1/XT/leaderV2/leaderboard/leaderboard/obs_manager/carla_gym_utils/")
...
class ObsManager_bird(ObsManagerBase):
    def __init__(self, obs_configs,ego_vehicles):
       # print(obs_configs)
        obs_configs = obs_configs['hero']['birdview']
        ...
        super(ObsManager_bird, self).__init__()
    ...
    def get_observation(self):
     #   ev_transform = self._parent_actor.vehicle.get_transform()  #ego_vehicles
        #print(f"self.ego_vehicles:{self.ego_vehicles}")
        ev_transform = self.ego_vehicles.get_transform()
        ev_loc = ev_transform.location
        ev_rot = ev_transform.rotation
      #  ev_bbox = self._parent_actor.vehicle.bounding_box
        ev_bbox = self.ego_vehicles.bounding_box
      ...
      #  print(f"carla.CityObjectLabel:{carla.CityObjectLabel}")
      #  attributes = [attr for attr in dir(carla.CityObjectLabel) if not attr.startswith('_')]
      #  attributes = [attr for attr in dir(carla.CityObjectLabel)]
        # 打印所有属性
        # for attr in attributes:
        #     print(attr)
        vehicle_bbox_list = self._world.get_level_bbs(carla.CityObjectLabel.Car)#最新版把车辆这个属性单独拿出去了？
