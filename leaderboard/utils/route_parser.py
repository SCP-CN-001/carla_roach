    ...
    def parse_routes_file(route_filename, routes_subset=''):
        ...
        for route in tree.iter("route"):
            ...
            for position in route.find('waypoints').iter('position'):
                ...
                #trans location to transform
                loc = carla.Location(x=float(position.attrib['x']),
                                     y=float(position.attrib['y']),
                                     z=float(position.attrib['z']))
                rot = carla.Rotation(
                    roll=float(0),
                    pitch=float(0),
                    yaw=float(0))  #ego_vehicles_action:
                positions.append(loc)
                trans_positions = carla.Transform(location=loc, rotation=rot)
                positions_roach.append(trans_positions)
            route_config.keypoints = positions
            route_config.keypoints_roach = positions_roach
