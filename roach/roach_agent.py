...
from roach.birdview.chauffeurnet import ObsManager
from roach.common_carla.run_stop_sign import RunStopSign
from roach.common_carla.traffic_light import TrafficLightHandler
from roach.planner import RoutePlanner
from roach.utils.common import get_absolute_path, load_entry_point
...
class RoachAgent(autonomous_agent.AutonomousAgent):
    def setup(
        ...
        ckpt = get_absolute_path("roach/models/checkpoint_L10.pth")
        ...
        if len(path_to_conf_file) > 2:
            string += (
                "_index" + str(path_to_conf_file[2]) + "_" + str(path_to_conf_file[3])
            )
        ...
        self.sensor_configs = OmegaConf.load(
            get_absolute_path(cfg["path_sensor_configs"])
        )
    ...
    def tick(self, input_data, timestamp):
        ...
        depth_back = cv2.cvtColor(
            input_data["depth_back"][1][:, :, :3], cv2.COLOR_BGR2RGB
        )
    ...
    def run_step(self, input_data, timestamp):
        ...
        tick_data, policy_input, rendered, target_gps, target_command, _ = self.tick(
        ...
        (
            ...
            features,
            cnn_feature,
        ) = self._policy.forward(policy_input, deterministic=True, clip_action=True)
        ...
        render_dict = {
            "rendered": rendered,
            "action": actions,
            "should_brake": str(should_brake),
        }

        render_img = self.im_render(render_dict)
        supervision_dict = {
            "action": np.array(
                [control.throttle, control.steer, control.brake], dtype=np.float32
            ),
            "value": values[0],
            "action_mu": mu[0],
    ...
    def _is_vehicle_hazard(self, vehicle_list):
        ...
        p1 = _numpy(self._ego_vehicle.get_location())
        s1 = max(
            10, 3.0 * np.linalg.norm(_numpy(self._ego_vehicle.get_velocity()))
    ...
    def save(
        ...
        Image.fromarray(tick_data["rgb_back"]).save(
            self.save_path / "rgb_back" / ("%04d.png" % frame)
        )
        Image.fromarray(tick_data["seg_front"]).save(
            self.save_path / "seg_front" / ("%04d.png" % frame)
        )
        Image.fromarray(tick_data["seg_left"]).save(
            self.save_path / "seg_left" / ("%04d.png" % frame)
        )
        Image.fromarray(tick_data["seg_right"]).save(
            self.save_path / "seg_right" / ("%04d.png" % frame)
        )
        Image.fromarray(tick_data["seg_back"]).save(
            self.save_path / "seg_back" / ("%04d.png" % frame)
        )
        Image.fromarray(tick_data["depth_front"]).save(
            self.save_path / "depth_front" / ("%04d.png" % frame)
        )
        Image.fromarray(tick_data["depth_left"]).save(
            self.save_path / "depth_left" / ("%04d.png" % frame)
        )
        Image.fromarray(tick_data["depth_right"]).save(
            self.save_path / "depth_right" / ("%04d.png" % frame)
        )
        Image.fromarray(tick_data["depth_back"]).save(
            self.save_path / "depth_back" / ("%04d.png" % frame)
        )
        Image.fromarray(render_img).save(self.save_path / "bev" / ("%04d.png" % frame))
        np.save(
            self.save_path / "bev_seg_label" / ("%04d.npy" % frame),
            tick_data["bev_seg_label"],
            allow_pickle=True,
        )
        with open(self.save_path / "supervision" / ("%04d.npy" % frame), "wb") as f:
            np.save(f, supervision_dict)

        current_inv_mat = np.array(
            self._ego_vehicle.get_transform().get_inverse_matrix()
        )
        relative_transform_mat = np.dot(
            current_inv_mat, np.array(self.prev_matrix)
        )  # 4 * 4
        transformed_prev_lidar_xyz = np.concatenate(
            [self.prev_lidar[:, :3], np.ones((self.prev_lidar.shape[0], 1))], axis=1
        )  # N * 4
        transformed_prev_lidar_xyz = np.einsum(
            "ij,kj->ki", relative_transform_mat, transformed_prev_lidar_xyz
        )
        transformed_prev_lidar_xyz = np.concatenate(
            [transformed_prev_lidar_xyz[:, :3], self.prev_lidar[:, 3][:, np.newaxis]],
            axis=1,
        )
        saved_lidar = np.concatenate(
            [transformed_prev_lidar_xyz, tick_data["lidar"]], axis=0
        )
    ...
    def get_target_gps(self, gps, compass):
        ...
        def gps_to_location(gps):
            ...
            location.y = -xy[
                1
            ]  ## Dose not matter, because no explicit judge left or right
            return location
        ...
        if (
            road_option_0 in [RoadOption.CHANGELANELEFT, RoadOption.CHANGELANERIGHT]
        ) and (
            road_option_1 not in [RoadOption.CHANGELANELEFT, RoadOption.CHANGELANERIGHT]
        ):
            road_option = road_option_1
        else:
            road_option = road_option_0

        return np.array(gps_point, dtype=np.float32), np.array(
            [road_option.value], dtype=np.int8
        )
    ...
    def _get_3d_bbs(self, lidar=None, max_distance=50):
        ...
        ego_matrix = np.array(self._ego_vehicle.get_transform().get_matrix()).astype(
            np.float32
        )
        ego_extent = self._ego_vehicle.bounding_box.extent
        ego_dx = (
            np.array([ego_extent.x, ego_extent.y, ego_extent.z]).astype(np.float32)
            * 2.0
        )
        ...
        for actor_type in ["vehicle", "walker", "traffic_light", "stop"]:
            ...
            for selected_actor in selected_actors:
                if selected_actor.get_location().distance(ego_loc) < max_distance:
                    if (actor_type != "vehicle") or (
                        selected_actor.id != self._ego_vehicle.id
                    ):
                        selected_actor_rotation = (
                            selected_actor.get_transform().rotation
                        )
                        selected_actor_matrix = np.array(
                            selected_actor.get_transform().get_matrix()
                        )
                        selected_actor_inv_matrix = np.array(
                            selected_actor.get_transform().get_inverse_matrix()
                        )
                        ...
                        if hasattr(selected_actor, "bounding_box"):
                            ...
                            dx = (
                                np.array(
                                    [
                                        selected_actor_extent.x,
                                        selected_actor_extent.y,
                                        selected_actor_extent.z,
                                    ]
                                ).astype(np.float32)
                                * 2.0
                            )
                        else:
                            dx = np.array([0.5, 0.5, 2]) * 2.0
                        yaw = selected_actor_rotation.yaw / 180 * np.pi
                        relative_yaw = yaw - ego_yaw
                        relative_rotation = [
                            selected_actor_rotation.roll - ego_rotation.roll,
                            selected_actor_rotation.pitch - ego_rotation.pitch,
                            selected_actor_rotation.yaw - ego_rotation.yaw,
                        ]
                        relative_rotation = [_ / 180 * np.pi for _ in relative_rotation]
                        relative_pos = np.dot(ego_inv_mat, selected_actor_matrix)[
                            :3, 3
                        ].astype(np.float32)
                        selected_actor_transform = selected_actor.get_transform()
                        selected_actor_control = (
                            selected_actor_velocity
                        ) = selected_actor_speed = selected_actor_brake = None
                        if actor_type in ["vehicle", "walker"]:
                            ...
                            selected_actor_speed = self._get_forward_speed(
                                transform=selected_actor_transform,
                                velocity=selected_actor_velocity,
                            )  # In m/s
                        ...
                        num_in_bbox_points = self.get_points_in_bbox(
                            ego_matrix, selected_actor_inv_matrix, dx, lidar
                        )
